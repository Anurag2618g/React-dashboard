  ...
  const handleSubmit = async(e) => {
    ...
    try {
        ...
        if (err.response.status == 400) {
          ...
          setError({flag: 'User with this email does not exists'});
        }
        else {
      ...
      <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        ...
        {flag ? (
            <div className="text-center">
              <p className="text-green-500 mb-6">If the email exists, a reset link has been sent.</p>
              <Link to='/login' className="text-sm mt-2 mx-2 text-indigo-400 hover:underline"> &lt; Back to login</Link>
            </div>
          ) : (
            <>
              {error.flag &&  <p className="text-red-500 mb-4 text-sm text-center">{error.flag}</p>} 
